...
Strategy.prototype.createSig = function(req) {
    return [req.method, req.originalUrl, req.headers.date].join('\n');
};
...
Strategy.prototype.checkDate = function(req) {
    if (!req.headers.date) return false;
    ...
    let headerDate = Date.parse(req.headers.date);
...
Strategy.prototype.authenticate = function(req, options) {
    ...
    let [garbage, publicKey, hashBase64] = parsedAuth;
    ...
    let verified = function (err, user, privateKey, info) {
        ...
        if (
            !user
            || !_this.checkDate(req)
            || _this.createHash(req, privateKey) !== hash
        ) {
